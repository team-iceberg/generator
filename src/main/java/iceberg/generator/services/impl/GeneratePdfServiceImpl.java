...
public class GeneratePdfServiceImpl implements GeneratePdfService {
  ...
  private static final Font FONT_TITLE_1 = FontFactory
      .getFont(FontFactory.HELVETICA, 20, BaseColor.BLACK);

  private static final Font FONT_TITLE_2 = FontFactory
      .getFont(FontFactory.HELVETICA, 16, BaseColor.BLACK);

  private static final Font FONT = FontFactory.getFont(FontFactory.HELVETICA, 12, BaseColor.BLACK);

  private static final Font FONT_BOLD = FontFactory
      .getFont(FontFactory.HELVETICA_BOLD, 12, BaseColor.BLACK);

  private static final Font FONT_SMALL = FontFactory
      .getFont(FontFactory.HELVETICA, 10, BaseColor.BLACK);

  private static final Font FONT_SMALL_BOLD = FontFactory
      .getFont(FontFactory.HELVETICA_BOLD, 10, BaseColor.BLACK);

  private static final Font FONT_SMALL_OBLIQUE = FontFactory
      .getFont(FontFactory.HELVETICA_OBLIQUE, 10, BaseColor.BLACK);

  @Override
  public File createFile(Family family) {
    File file;
    try {
    ...
    try (FileOutputStream fileOut = new FileOutputStream(file)) {
      ...
      throw new ServiceException("échec de la création du pdf", e);
    }
    return file;
  }

  private void generatePdf(FileOutputStream fos, Family family) throws IOException,
...
//        PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("test_fiche_réinscription.pdf"));
    ...
    Phrase footer = new Phrase(
        "Association Les Sympathiques - Place Robert Devos, 59940 Neuf-Berquin - associationslessympathiques@gmail.com",
        FONT_SMALL);
...
//        document.getJavaScript_onLoad();
  ...
  private void setHeader(Document document, Family family)
  ...
  private PdfPCell getTitle(Family family) {
    Chunk title = new Chunk(
        "Fiche de réinscription année : " + getYears() + "\n____________________", FONT_TITLE_1);
    ...
    Chunk generalInformation = new Chunk(familyName + guardian + address + city + phone + mail,
        FONT_BOLD);
  ...
  private void setMembers(Document document, List<Membership> memberships)
    ...
    Chunk chunkSmall = new Chunk(
        "(Pour toute nouvelle inscription voir la liste d'attente lors du dépot de la fiche de réinscription)",
        FONT_SMALL_OBLIQUE);
  ...
  private void addMemberTableHeader(PdfPTable table) {
    Stream
        .of("NOM, Prénom", "Date de naissance", "1ere Inscription", "Attestation ?", "Commentaire")
        .forEach(columnTitle -> {
  ...
  private void setRules(Document document) throws DocumentException {
    ...
    Chunk ruleText = new Chunk(
        "J’ai pris connaissance du règlement de l’association “Les Sympathiques” et je m’engage à le respecter.\n"
  ...
  private void setPayment(Document document) throws DocumentException {
    ...
    Chunk chunk = new Chunk("(Paiement possible en plusieurs fois seulement si fratrie)",
        FONT_SMALL_OBLIQUE);
    Paragraph paragraph = new Paragraph();
    paragraph.add(chunk);
    paragraph.setAlignment(Element.ALIGN_CENTER);
